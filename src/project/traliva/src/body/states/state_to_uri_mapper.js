...
StateToUriMapper.prototype.updateState = function(){
    ...
    //    return;
    console.log('prevAr:' + JSON.stringify(this._prevAr));
    ...
    console.log('%c' + JSON.stringify(ar), 'color:red');
    console.log('%c' + JSON.stringify(prevArCandidate), 'color:blue');

    var treeRefOld = this._statesObj.tree;
    var treeRef = this._statesObj.tree;
    var nodeDescr, nodeDescrOld;
    var p, pOld;
    console.log('%cinit new: '+JSON.stringify(ar), 'color:red');
    console.log('%cinit old: '+JSON.stringify(this._prevAr), 'color:blue');
    while(ar.length){
        p = null;
        pOld = null;
        tmp = ar.shift();
